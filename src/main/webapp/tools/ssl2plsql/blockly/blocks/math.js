...
Blockly.Extensions.registerMutator('math_modes_of_list_mutator',
    ...
    Blockly.Extensions.register('dynamic_class_extension',
    function() {
      this.getInput('INPUT')
        .appendField(new Blockly.FieldDropdown(
          function() {
            ...
            for(var i = 0; i < models.length; i++) {
              if(models[i].hasOwnProperty('class')) {
                var val = models[i].class;
  ...
  Blockly.Extensions.register('dynamic_attribute_extension',
    function() {
      this.getInput('INPUT')
        .appendField(new Blockly.FieldDropdown(
          function() {
            ...
            for(var i = 0; i < models.length; i++) {
              if(models[i].hasOwnProperty('class')) {
                var classname = models[i].class;
                var attributes = models[i].attributes;
                for(var j = 0; j < attributes.length; j++) {
                  var val = attributes[j].name;
                  options.push([val + '(' + classname + ')', val]);
                }
              }
            }
            return options;
